# Check Tag_RS Android v0.13

## การแก้ใน v0.13 — DNTH DISC และช่องว่าง

- เพิ่มกฎ DISC ก่อนกฎลูกค้าอื่น อ่านโครงสร้างทั้งรายการ ไม่ค้นรหัสใดก็ได้ที่ตรงกับ BOX
- ตัวอย่างใหม่: รหัสแรก TG028382-502C, รหัสก่อนจำนวน TG028993-590A, จำนวน 0000040, รหัสท้าย TG028382-502C → ใช้ TG028993-590A เปรียบเทียบ
- ตรวจรหัสอ้างอิงต้นและท้ายว่าตรงกัน แต่ไม่ใช้เป็น Part No. ของ BOX ในแบบใหม่
- รูปแบบ DISC เดิมที่มีรหัสเดียวก่อนจำนวนยังใช้รหัสนั้น และตรวจสำเนาท้ายรายการ
- รูปแบบที่รองรับจากตัวอย่าง: DISC ตามด้วยเลข/ช่องว่าง, รหัส TG หรือ TGY, รหัสที่สอง (ถ้ามี), จำนวน 7 หลัก, C07, หมายเลข Tag, lane T-เลข (ถ้ามี), เลข D/O, รหัสอ้างอิงซ้ำ, 01
- ชื่อเชิงธุรกิจของช่องยังไม่มี specification ยืนยัน ใช้ลำดับช่องจาก RAW DATA ที่ผู้ใช้ให้ ห้ามขยายไปยังรูปแบบอื่นด้วยการเดา
- ไม่สนใจช่องว่าง Unicode เช่น NBSP ใน Part No. แต่คง RAW DATA ที่บันทึกเดิมและไม่ลบตัวคั่นทั้งรายการก่อนแยกช่อง
- DNTH เทียบ suffix เต็ม เช่น 590A ไม่เท่ากับ 590B; กฎ JTEKT ก่อนขีดแรกยังเดิม
- เพิ่มการอ่าน I-prefixed BOX ที่ suffix เป็นตัวอักษร เช่น ITG028993-590A
- versionCode 14 / versionName 0.13.0

### ผลตรวจและข้อจำกัด

ตรวจ regex ที่สกัดจาก Source ด้วย Java บน JVM ผ่าน 11 กรณี: ตัวอย่างใหม่, Unicode, แบบเดิม TG/TGY, ช่องที่เลือกผิดรูปแบบ, จำนวนหาย, รหัสเพิ่มเติม, รหัสอ้างอิงขัดกัน และรายการขาดตอน
เพิ่ม JUnit ใน TagParserTest.kt แต่ยังไม่ได้ compile Kotlin, รัน JUnit หรือ Build Android ในเครื่องนี้ เนื่องจากไม่มี compiler/Android SDK และดาวน์โหลด dependency ไม่ได้ ต้องรัน GitHub Actions และทดสอบ PM75 ก่อนใช้จริง

### งานก่อนหน้าที่ยังไม่รวมในชุดนี้

- Flow พนักงาน → Stand → Kanban 1 ใบ → Box หลายใบ → BOX ครบ ยังไม่ได้แก้ UI/สถานะการทำงาน; ชุดนี้ยังเป็น Flow เดิมของ v0.12 ตามด้านล่าง
- ขั้นตอนยืนยันส่งงานผิดแล้วบังคับส่งอีเมลยังไม่ได้เพิ่ม; การเปิด Outlook อย่างเดียวไม่ยืนยันว่าเมลถูกส่งจริง
- ไม่มี APK แนบใน ZIP นี้
...
- สำหรับ JTEKT/JATH เปรียบเทียบเฉพาะข้อความก่อน `-` ตัวแรกใน Stand, Box และ Kanban
...
## กติกา Kanban ใน v0.13

- Aisin: ตัดเลข `0` นำหน้าที่เป็นรหัส Kanban ออก แล้วใช้ Part No. เต็มที่เหลือ
- DNTH แบบ DISC: อ่านช่องข้อมูลตามรายละเอียด v0.13 ด้านบน; แบบที่ไม่ขึ้นต้น DISC ยังใช้กฎเดิมที่ต้องพบรหัสตัวเลขซ้ำและตรงกันอย่างน้อย 2 ครั้ง; BOX TAG ที่ขึ้นต้น I รองรับ suffix 4 ตัวอักษร/ตัวเลข
- SNSS: ตัวอย่าง `7521T0376 260805 80 CLM012` จะใช้ `7521T0376` เปรียบเทียบกับ STAND/BOX TAG
- JTEKT/JATH: ไม่สนใจ `-` และข้อความทั้งหมดด้านหลังในทุก Tag เช่น `JGC123456-40` เทียบเป็น `JGC123456`
...
7. ที่หัวข้อ `Artifacts` กดดาวน์โหลด `Check_Tag_RS_v0.13_APK`
8. แตก ZIP ที่ดาวน์โหลด จะได้ `Check_Tag_RS_v0.13.apk` สำหรับติดตั้งบน PM75
